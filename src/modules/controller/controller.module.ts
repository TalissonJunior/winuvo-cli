...
export class ControllerModule extends BaseModule {
    ...
    create(name: string, modelName: string, callback: BaseCallback): void {
        ...
        var businessExtension = ValidateService.capitalizeFirstLetter(name) + this.config['businessPath']['suffixExtension'];
        var controllerExtension = ValidateService.capitalizeFirstLetter(name) + this.config['controllerPath']['suffixExtension'];
        ...
        if (!fs.existsSync(path.join(modelPath, modelExtension))) {
            ...
            if (this.schematics.createFile(controllerPath, this._createControllerFileTemplate(name, modelName), controllerExtension)) {
